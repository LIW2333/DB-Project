...
struct BplusNodeInt {
  ...
  bool isleaf;  // will be true if the node is leaf node, otherwise it will be
                // false
};
...
class IndexManager : public BufferManager {
 private:  // B+ tree data and operation
  ...
  int createBplusTree(TargetInt);  // create a new B+ tree, including creating
                                   // the index file and config file
...
void IndexManager::drop(Target<T> t) {
  if (file_name == t.getIndexFileName()) {
    root = -1;
    file_name = "";
    indexFileName = "";
    configFileName = "";
    rootTable.erase(file_name);
