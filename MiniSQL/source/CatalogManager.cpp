...
catalogManager::catalogManager() { readCatalog(); }

catalogManager::~catalogManager() { writeCatalog(); }
...
string makeTableName(string tablename) { return tablename; }
...
void catalogManager::readCatalog() {
  ...
  if (logfile.good()) {
    ...
    if (s.length() != 0) {
      ...
      try {
        copy(istream_iterator<TableNode>(logfile),
             istream_iterator<TableNode>(), back_inserter(this->tableList));
  ...
  if (logfile.good()) {
    copy(istream_iterator<indexNode>(logfile), istream_iterator<indexNode>(),
         back_inserter(this->indexList));
    logfile.close();
  }
}
...
void catalogManager::writeCatalog() {
  ...
  if (logfile.good()) {
    copy(this->tableList.begin(), this->tableList.end(),
         ostream_iterator<TableNode>(logfile));
    logfile.close();
  }
  logfile.open("indexLog.log", ios::out);
  if (logfile.good()) {
    copy(this->indexList.begin(), this->indexList.end(),
         ostream_iterator<indexNode>(logfile));
    logfile.close();
  }
}
...
bool catalogManager::createIndex(string indexname, string tablename,
                                 int columns) {
  ...
  if (indexfile.good() && configfile.good())  // already exists
  {
    ...
    for (tite = this->tableList.begin(); tite != this->tableList.end();
      ...
      if (tite->tableName == tablename) {
        ...
        for (int i = 0; i < columns; ++i) ++aite;
        attrname = aite->attrName;
        aite->hasIndex = true;  // set hasIndex
        if (aite->isPrimary) indexname = string("$") + indexname;
      }
    }
    //		indexfile.close();
...
bool catalogManager::doesIndexExist(string indexname) {
  ...
  for (iite = this->indexList.begin(); iite != this->indexList.end(); ++iite) {
    if (iite->indexName == indexname) return true;
  }
  return false;
}
...
bool catalogManager::doesAttrExist(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return true;
      }
    }
  }
  return false;
...
int catalogManager::getAttrType(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return aite->type;
      }
    }
  }
  return -1;  // not found
...
int catalogManager::getAttrOffset(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return aite->offset;
      }
    }
  }
  return -1;  // not found
...
int catalogManager::getAttrLength(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return aite->length;
      }
    }
  }
  return -1;  // not found
...
attrNode catalogManager::getAttrInfo(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return *aite;
      }
    }
  }
  return *aite;
...
list<string> catalogManager::getIndexOfTable(string tablename) {
  ...
  list<string> sv;
  for (iite = this->indexList.begin(); iite != this->indexList.end(); ++iite) {
    if (iite->tableName == tablename) sv.push_back(iite->indexName);
  }
  return sv;
}
...
int catalogManager::getAttrNum(string tablename, string attrname) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
      for (aite = tite->attrList.begin(); aite != tite->attrList.end();
           ++aite) {
        if (aite->attrName == attrname) return num;
        ++num;
      }
    }
  }
...
bool catalogManager::deleteIndex(string indexname, string tablename)
{
        ...
        for (iite = this->indexList.begin(); iite != this->indexList.end();)
        {
                if (iite->tableName == tablename&&iite->indexName == indexname)
                {
                        ...
                        string file_name = makeIndexName(tablename, indexname);
                        remove(file_name.c_str());
                        file_name = makeConfigName(tablename, indexname);
                        remove(file_name.c_str());
                }
                else
                {
    ...
    if (iite->indexName == indexname) {
      for (tite = this->tableList.begin(); tite != this->tableList.end();
        ...
        if (tite->tableName == iite->tableName) {
          for (aite = tite->attrList.begin(); aite != tite->attrList.end();
               ++aite) {
            if (aite->attrName == iite->attribute) aite->hasIndex = false;
          }
        }
      }
      string file_name = makeIndexName(iite->tableName, indexname);
      remove(file_name.c_str());
      file_name = makeConfigName(iite->tableName, indexname);
      remove(file_name.c_str());
      iite = this->indexList.erase(iite);
    } else {
      ++iite;
...
bool catalogManager::deleteTable(string tablename) {
  ...
  for (tite = this->tableList.begin(); tite != this->tableList.end();) {
    if (tite->tableName == tablename) {
      ...
      string file_name = makeTableName(tablename);
      remove(file_name.c_str());
    } else {
      ++tite;
    }
  ...
  for (iite = this->indexList.begin(); iite != this->indexList.end();) {
    if (iite->tableName == tablename) {
      string file_name = makeIndexName(tablename, iite->indexName);
      remove(file_name.c_str());
      file_name = makeConfigName(tablename, iite->indexName);
      remove(file_name.c_str());
      iite = this->indexList.erase(iite);
    } else {
      ++iite;
...
bool catalogManager::recordDelete(string tablename,
                                  int num)  //删除条目数量，删除成功返回1
{
  list<TableNode>::iterator tite;
  for (tite = this->tableList.begin(); tite != this->tableList.end(); ++tite) {
    if (tite->tableName == tablename) {
...
void catalogManager::showTables() {
  for (list<TableNode>::iterator tite = this->tableList.begin();
       tite != this->tableList.end(); ++tite) {
    cout << "=================================================================="
            "============"
         << endl;
    int column = 0;
    ...
    cout << "------------------------------------------------------------------"
            "------------"
         << endl;
    for (list<attrNode>::iterator aite = tite->attrList.begin();
         aite != tite->attrList.end(); ++aite) {
      cout << aite->attrName << (aite->attrName.size() < 8 ? "\t" : "") << '\t'
           << isTrue(aite->isPrimary) << '\t' << isTrue(aite->isUnique) << '\t';
      switch (aite->type) {
        case 3:
          cout << "float"
               << "\t\t";
...
void catalogManager::showIndexes() {
  ...
  cout << "--------------------------------------------------------------------"
          "----------"
       << endl;
  for (list<indexNode>::iterator iter = this->indexList.begin();
       iter != this->indexList.end(); ++iter) {
    cout << iter->indexName << '\t' << (iter->indexName.size() >= 8 ? "" : "\t")
         << iter->tableName << '\t' << (iter->tableName.size() >= 8 ? "" : "\t")
         << iter->attribute << endl;
  }
  cout << "===================================================================="
          "=========="
