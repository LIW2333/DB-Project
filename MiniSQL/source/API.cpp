...
void merge_index(vector<vector<index_info>>& index,
  ...
  for (it = ++index.begin(); it != index.end(); it++) {
    ...
    while (a != result.end() && b != it->end()) {
      if (a->num == b->num && a->offset == b->offset) {
        a++;
        b++;
        continue;
      } else if (compare(*a, *b))
    ...
    if (a != result.end())
      while (a != result.end()) a = result.erase(a);
  }
}
  ...
  switch (op) {
      ...
      switch (type) {
        ...
        case MINI_INDEX:
          try {
            ...
            if (createIndex(tablename, inter.getIndexName(), attribute,
                            inter.indexOnWhichColumn())) {
              /*select all the existed tuples in table and add them into the
               * index*/
              /*addition needed*/
    ...
    case MINI_INSERT:
      /*make a vector of attrAndvalue*/
      {
        ...
        switch (result) {
          case -1:
            cout << "abort because of duplication!" << endl;
            break;
...
int API::createTable(
    string tablename,
    vector<attrNode>
        attributes)  // throw(alreadyExist,CreationFailureOnCatalog,)
{
  // return value 0 -1 1
...
int API::createIndex(string tablename, string indexname, string attribute,
                     int column)  // throw(alreadyExist,CreationFailureOnBplus)
{
  ...
  if (flag)
    ...
    try {
      flag = 0;
      if (!catlog.createIndex(indexname, tablename, column)) return -1;
      /*
      ...
      switch (
          attrPro
              .type) {  // for different t ype , create a corressponding index.
        case MINI_FLOAT:
      ...
      vector<attrValue> all_tuples =
          record_manager_.select_attr(tablename, attr_list, column);
      cout << "selecting finish" << endl;
      vector<attrValue>::iterator tuples =
          all_tuples.begin();  // get all the values in the attribute
...
int API::Insert(string tablename, vector<Union> data) {
  ...
  try {
    /*find  the unique and primary value in index
     if they already exist,abort,if not,change the property unique or primary to
     false. If there is no index, send it to recordmanager to linear scan.
     */
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      if (tmp.isPrimary && indexname != "" ||
          tmp.isUnique && indexname != "")  // test if already exists
      {  // try to find if the value is duplicated in indexmanager
        ...
        switch (attr.type) {
          case MINI_STRING:
            ...
            if (!exist.empty()) {
              cout << "the " << attr.attrName << " can't have duplicate value!"
                   << endl;
              return -1;
            }  // already exists
            break;
        ...
        if (tmp.isUnique && indexname == "") {
          // try to find if the value is duplicated by getting all the values in
          // table and traversing them
          vector<attrNode> tttmp = ListToVector(catlog.getAttrList(tablename));
          vector<attrValue> all_tuples =
              record_manager_.select_attr(tablename, tttmp, i);
          vector<attrValue>::iterator tuple = all_tuples.begin();
          for (; tuple != all_tuples.end(); tuple++) {
            switch (attr.type) {
              case MINI_INT:
                if (data[i].n == tuple->value.n) {
                  cout << "the " << attr.attrName
                       << "can't have duplicate value!";
                  return -1;
                }
                break;
              case MINI_FLOAT:
                if (data[i].f == tuple->value.f) {
                  cout << "the " << attr.attrName
                       << "can't have duplicate value!";
                  return -1;
                }
    ...
    vector<attrNode> zkw =
        ListToVector<attrNode>(catlog.getAttrList(tablename));
    ...
    for (; insert_it != attlist.end(); insert_it++, i++) {
      string indexname = catlog.getIndexByAttrID(tablename, i);
      if (indexname !=
          "")  // insert the new tuple into the corressponding indices
      {
        Target<string> tmp_str(indexname);
        Target<int> tmp_int(indexname);
        Target<float> tmp_float(indexname);
...
int API::Delete(string tablename, vector<TreeNode> conditions) {
  ...
  try {
    ...
    vector<vector<deleted_node>> deleted_attr = record_manager_.delete_tuple(
        tablename, conditions, attributes, merge_result);
    /*
     delete the index according to the recordmanager's return vector.
     */
    vector<vector<deleted_node>>::iterator deleted_attr_it =
        deleted_attr.begin();
...
int API::Select(string tablename, vector<TreeNode> conditions) {
  try {
    ...
    // auto d = duration_cast<duration<double, micro>>(end - start);
    // print_time += d.count();
    ...
    // d = duration_cast<duration<double, micro>>(end - start);
    // print_time += d.count();
