  ...
  while (loop) {
    ...
    try {
      ...
      */
      ...
      for (int i = 0; i < statement.size(); ++i) cout << statement[i] << ' ';
      cout << endl;
    } catch (string file) {
      streambuf *backup;
      ifstream fin;
      ...
      auto dura = duration_cast<duration<double, milli>>(end - start);
      // auto end = chrono::system_clock::now();
      // auto duration = chrono::duration_cast<chrono::microseconds>(end -
      // start);

      cout << "FINISH , USE TIME : " << dura.count() - print_time << "ms"
           << endl;
...
void Interpreter::readCommand(bool cmd)  // if a statement is
{
  ...
  while (line[trim] == '\n' || line[trim] == '\t' || line[trim] == '\r' ||
         line[trim] == ' ')
    trim--;
  line = line.substr(0, trim + 1);
  // to lower case;
  for (int i = 0; i < line.size(); ++i)
    if (line[i] <= 'Z' && line[i] >= 'A') line[i] += 32;
  int place;                             // place of ';'
  ...
  if (place != line.size())  // there may be more than one ';'s
    throw SyntaxException(
        "there should be exactly one ';' locating at the end of a statement.");
}
...
int Interpreter::split(string line) {
  ...
  for (int i = 0; i < line.size();) {
    ...
    t = line.substr(i, j - i);
    if (t.size()) statement.push_back(t);
...
Interface Interpreter::parse() {
  ...
  if (statement[0] == "create") {
    if (statement.size() < 2 ||
        statement[1] != "table" && statement[1] != "index")
      throw SyntaxException("fail to create something.");
    // create index
    if (statement[1] == "index") {
      if (statement.size() != 8 || statement[3] != "on" ||
          statement[5] != "(" || statement[statement.size() - 1] != ")")
        throw SyntaxException(
            "\'create index\' should be in the format: create index "
            "<index_name> on <talbe_name> ( <colume_name> ).");
      ...
      if (catalog.doesIndexExist(statement[2]))
        throw SyntaxException("index \'" + statement[2] +
                              "\' has already existed.");
      if (!regex_match(statement[2], regex("[_|a-z|A-Z](\\w*)")))
        throw SyntaxException(string("\'") + statement[2] +
                              "\' isn't a legal index name.");
      ...
      if (attr.isPrimary)
        throw SyntaxException(
            statement[6] +
            " is a primary key, and a default index has been created.");
      if (!attr.isUnique)
    ...
    // create table
      ...
      if (statement.size() < 7 || statement[3] != "(" ||
          statement[statement.size() - 1] != ")")
        throw SyntaxException(
            "\'create table\' should be in the format: create table "
            "<table_name> ( <attr_list> [, primary key(attr_name)]).");
      if (catalog.doesTableExist(statement[2]))
        throw SyntaxException("table \'" + statement[2] +
                              "\' has already existed.");
      if (!regex_match(statement[2], regex("[_|a-z|A-Z](\\w*)")))
        throw SyntaxException(string("\'") + statement[2] +
                              "\' isn't a legal table name.");
      ...
      for (int i = 0; i < attributes.size(); ++i) {
        ...
        if (attribute[0] == "primary" && i == attributes.size() - 1) {
          if (attribute.size() != 5 || attribute[1] != "key" ||
              attribute[2] != "(" || attribute[4] != ")")
            throw SyntaxException(
                "\'primary key\' should be in the format: primary key ( "
                "<attr_name> ).");
        ...
        // normal keys
          ...
          if (attribute[1] == "int") {
            if (attribute.size() == 3 && attribute[2] != "unique")
              throw SyntaxException(
                  "INT attribute should be in the format: <attr_name> int "
                  "[unique].");
            ...
            if (attribute.size() == 3 && attribute[2] != "unique")
              throw SyntaxException(
                  "FLOAT attribute should be in the format: <attr_name> float "
                  "[unique].");
            else if (attribute.size() == 3 && attribute[2] == "unique")
              attr.isUnique = true;
            else if (attribute.size() != 2)
            ...
            if (attribute.size() == 6 && attribute[5] != "unique")
              throw SyntaxException(
                  "CHAR attribute should be in the format: <attr_name> "
                  "char(<len>) [unique].");
            else if (attribute.size() == 6 && attribute[5] == "unique")
              attr.isUnique = true;
            else if (attribute.size() != 5 || attribute[2] != "(" ||
                     attribute[4] != ")")
              throw SyntaxException(
                  "CHAR attribute should be in the format: <attr_name> "
                  "char(<len>) [unique].");
            ...
            if (!regex_match(attribute[3], integer) || attribute[3].size() > 4)
              throw SyntaxException(
                  string("\'") + attribute[3] +
                  "\' should be a positive integer less than 10000.");
            ...
            throw SyntaxException(string("\'") + attribute[1] +
                                  "\' unknown type.");
    ...
    if (statement[1] == "table") {
      if (!catalog.doesTableExist(statement[2]))
        throw SyntaxException(string("table \'") + statement[2] +
                              "\' not found.");
      if (!regex_match(statement[2], regex("[_|a-z|A-Z](\\w*)")))
        throw SyntaxException(string("\'") + statement[2] +
                              "\' isn't a legal table name.");
    ...
    // drop index
      ...
      if (!catalog.doesIndexExist(statement[2]))
        throw SyntaxException(string("index \'") + statement[2] +
                              "\' not found.");
      if (!regex_match(statement[2], regex("[_|a-z|A-Z](\\w*)")))
        throw SyntaxException(string("\'") + statement[2] +
                              "\' isn't a legal table name that can be drop.");
      interface.setObject(MINI_INDEX);
      interface.setIndexName(statement[2]);
    ...
    if (statement.size() < 4 || statement[1] != "*" || statement[2] != "from" ||
        (statement.size() > 4 &&
         (statement[4] != "where" || statement.size() == 5)))
      throw SyntaxException(
          "\'select\' should be in the format: select * from <table_name> "
          "[where <conditions>]");
    if (!catalog.doesTableExist(statement[3]))
      throw SyntaxException(string("table \'") + statement[3] +
                            "\' not found.");
    ...
    if (statement.size() < 7 || statement[1] != "into" ||
        statement[3] != "values" || statement[4] != "(" ||
        statement[statement.size() - 1] != ")")
      throw SyntaxException(
          "\'insert\' should be in the format: insert into <table_name> values "
          "(<value_list>)");
    if (!catalog.doesTableExist(statement[2]))
      throw SyntaxException(string("table \'") + statement[2] +
                            "\' not found.");
    ...
    for (int i = 5; i < statement.size() - 1; i += 2) {
      if (statement[i] == ",")
        throw SyntaxException(
            "if you want to insert a null value, please explicitly use "
            "\'null\'.");
    ...
    int i = 0;
    // test each value;
    for (list<attrNode>::iterator iter = attribute.begin();
         iter != attribute.end(); ++iter, ++i) {
      Union u;
      if (values[i] == "null") {
        u.isNull = true;
        interface.addData(u);
      ...
      switch (iter->type) {
        case MINI_INT: {
          ...
          if (!regex_match(values[i], pattern))
            throw SyntaxException(iter->attrName +
                                  " is defined as an INT while \'" + values[i] +
                                  "\' isn't.");
        ...
        case MINI_FLOAT: {
          ...
          if (!regex_match(values[i], pattern))
            throw SyntaxException(iter->attrName +
                                  " is defined as a FLOAT while \'" +
                                  values[i] + "\' isn't.");
          double f = atof(values[i].c_str());
          if (f == HUGE_VAL) throw SyntaxException("number out of range.");
          u.f = (float)f;
          break;
        }
        case MINI_STRING: {
          if (values[i].size() <= 1 || values[i][0] != '\'' ||
              values[i][values[i].size() - 1] != '\'')
            throw SyntaxException(values[i] +
                                  " can't be identified as a string.");
    ...
    if (statement.size() < 3 || statement[1] != "from" ||
        (statement.size() > 3 &&
         (statement[3] != "where" || statement.size() == 4)))
      throw SyntaxException(
          "\'delete\' should be in the format: delete from <table_name> [where "
          "<conditions>]");
    if (!catalog.doesTableExist(statement[2]))
      throw SyntaxException(string("table \'") + statement[2] +
                            "\' not found.");
    ...
    if (statement.size() != 2)
      throw SyntaxException(
          "\'execfile\' should be in the format: execfile <file_name>.");
...
void Interpreter::processCondition(string table, vector<string> &statement,
                                   int index, Interface &interface) {
  ...
  while (index < statement.size()) {
    ...
    if (condition.size() != 3 || !op.count(condition[1]))
      throw SyntaxException(
          "condition should be in format: <variable> <op> <value>.");
    if (!catalog.doesAttrExist(table, condition[0]))
      throw SyntaxException(string("column \'") + condition[0] +
                            "\' not found.");
    // get attribute type
    ...
    if (condition[2] == "null") {
      ...
      if (condition[1] != "=" || condition[1] != "<>")
        throw SyntaxException(
            "Cannot compare a value with null using <, >, <= or >=.");
    } else
      switch (type) {
        case MINI_INT: {
          // //contains only numbers
          ...
          if (!regex_match(condition[2], pattern))
            throw SyntaxException(condition[0] +
                                  " is defined as an INT while \'" +
                                  condition[2] + "\' isn't.");
        ...
        case MINI_STRING: {
          if (condition[2].size() <= 1 || condition[2][0] != '\'' ||
              condition[2][condition[2].size() - 1] != '\'')
            throw SyntaxException(condition[2] +
                                  " can't be identified as a string.");
